...
export async function POST(req: NextRequest) {
  ...
  if (purchase) {
    ...
    if (stageKey === "missao_final") {
      // Gatilho 1 de emissão do certificado (o outro é a aprovação no quiz de
      // validação em /api/quiz). Cobre quem passa no quiz ANTES de concluir a
      // trilha. Ver issueCertificateForToken.
