...
## Próximo passo
Nada a executar — frente concluída (plano já executado, migration `0020`
aplicada em produção). Minors deferidos (não bloqueantes): fetch em gated view;
reply a `topicId` inexistente retorna 500. Evolução futura (respostas
aninhadas) está na Trilha D do programa de lançamento final.
