...
### L-053 · Auditoria com múltiplos agentes de navegador em paralelo pode derrubar o próprio site sendo auditado
- **Gatilho:** `subagentes`, `deploy`
- **Não faça:** disparar vários subagentes fazendo navegação ao vivo (Chrome) contra produção ao mesmo tempo, sem throttle, quando o banco roda num compute free-tier pequeno (Supabase Nano: ~0.5GB RAM/CPU compartilhada). 5 agentes em paralelo testando `/feed`, `/forum`, `/dashboard/*/conteudo/*` etc. levaram o compute a 85% CPU/85% memória e o site inteiro (todas as rotas que tocam o banco) ficou fora do ar por >20 min com timeout de 300s — inclusive pra usuários reais, não só pros agentes. Os sintomas ("fórum sempre redireciona pro feed", "deep-link de conteúdo cai no feed") pareciam bugs de produto mas eram só requests pendurados por trás — não existe nenhum `redirect()` nessas rotas no código, foi confirmado por leitura direta.
- **Faça:** para uma auditoria com múltiplos agentes de navegador contra produção, ou (a) escalonar/throttle os agentes (não todos batendo ao mesmo tempo), ou (b) rodar contra um preview deployment/dev local em vez de produção quando o objetivo é só achar bugs de UI, ou (c) se tiver que ser produção, avisar o usuário antes que pode haver degradação temporária. Ao investigar "site lento"/travando, checar runtime logs da Vercel por `504`/`Task timed out` ANTES de suspeitar de bug de código — e o painel Supabase → Settings → Infrastructure mostra CPU/memória do compute em tempo real; reiniciar o projeto (`Restart project`) resolve overload sem perda de dado quando o compute trava em vez de se recuperar sozinho.
- **Fonte:** sessão 2026-09-10, auditoria de 5 agentes paralelos (2 acharam falso-positivos "de redirect"; causa raiz achada via Vercel runtime logs + painel Supabase Infrastructure, resolvida com restart do projeto).
